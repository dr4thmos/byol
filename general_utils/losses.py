...
class FocalLoss(nn.Module):
    ...
    def forward(self, x: Tensor, target: Tensor) -> Tensor:
        ...
        pt = self._calc_pt(target, x)
        focal = 1 - pt
        nll = -torch.log(self.eps + pt)
        #nll = nll.masked_fill(mask, 0)
        loss = weights * (focal ** self.gamma) * nll
        return self._reduce(loss, weights)

    def _reduce(self, x: Tensor,weights: Tensor) -> Tensor:
        if self.reduction == 'mean':
            return x.sum() / weights.sum()
