...
def main():
    ...
    train_dataset       = Zorro(targ_dir = data_path, transform = data_transform,
                          datalist="train.json"
        )

    validation_dataset  = Zorro(targ_dir = data_path, transform = data_transform,
    ...
    eval_loader         = DataLoader(eval_dataset, batch_size=config['trainer']['batch_size'],
                                num_workers=config['trainer']['num_workers'], drop_last=False, shuffle=True)
    ...
    linear_classificator = torch.nn.Linear(
        pretrained_model.repr_shape, train_dataset.num_classes).to(device)
    ...
    accuracy = Accuracy(task="multiclass", num_classes=train_dataset.num_classes, top_k=1).to(device)
