...
def main():
    ...
    data_transform_val      = get_data_transforms_test_robin(**config['network'])
    ...
    validation_dataset  = Robin(
        ...
        datalist = "info_only_meerkat.json"
    )
    ...
    linear_classificator    = torch.nn.Linear(
        backbone.repr_shape, train_dataset.num_classes).to(device)
    ...
    writer = SummaryWriter(log_dir=os.path.join(args.folder, args.experiment))

    grid_done = False
    for test_epoch in range(args.epochs):
        print(f"train Iteration: {test_epoch}")
        
        for idx, data in enumerate(train_loader):
            batch_views     = data[0].to(device)
            gtruth          = data[1].to(device)

            if not(grid_done):
                grid = torchvision.utils.make_grid(batch_views[:64])
                writer.add_image('views_paired_downstream', grid, global_step=epoch)

                grid_done = True
                print("Grid done")

            with torch.no_grad():
                features    = backbone(batch_views, repr=True)
    ...
    writer.add_scalars('val_accuracy', {'val_accuracy': accuracy}, global_step=epoch_counter)
    linear_classifier.reset_parameters()

    if accuracy > max_acc:
        max_acc = accuracy
        save_model(os.path.join(log_dir, "best_model.pt"))
